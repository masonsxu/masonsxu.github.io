...
const DESCRIPTION_TEXT = '我是专注 Go 后端开发的工程师，长期聚焦分布式系统架构、云原生基础设施与数据湖平台。主导 Python 单体到 CloudWeGo 微服务架构的整体转型，独立设计并交付 10+ 微服务的分布式数据平台。'
const DESCRIPTION_FONT = "400 clamp(0.9375rem, 1.5vw, 1.125rem) 'Inter', system-ui, sans-serif"
...
export default function HeroEditorial() {
  ...
  const dragRef = useRef<{ orbIndex: number; offsetX: number; offsetY: number } | null>(null)
  ...
  const computeLayout = useCallback(() => {
    ...
    const headlinePrepared = prepareWithSegments(HEADLINE_TEXT, headlineFit.font)
    ...
    const bodyTop = headlineRegion.y + headlineRegion.height + (isNarrow ? 16 : 24)
    ...
    const descTop = bodyTop + bodyLines.length * BODY_LINE_HEIGHT + (isNarrow ? 12 : 16)
    const descMaxHeight = Math.max(120, 480 - descTop)
    const descPrepared = prepareWithSegments(DESCRIPTION_TEXT, DESCRIPTION_FONT)
    ...
    const descLines = layoutTextWithObstacles(
      descPrepared,
      { x: GUTTER, y: descTop, width: stageWidth - GUTTER * 2, height: descMaxHeight },
      DESCRIPTION_LINE_HEIGHT,
      layoutObstacles,
    )

    allPositionedLines.push(...descLines.map(l => ({ ...l, className: 'hero-desc-line' })))
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      for (let i = 0; i < orbs.length; i++) {
        ...
        if (orb.cx - orb.r < 0) { orb.cx = orb.r; orb.vx = Math.abs(orb.vx) }
        if (orb.cx + orb.r > stageWidth) { orb.cx = stageWidth - orb.r; orb.vx = -Math.abs(orb.vx) }
        if (orb.cy - orb.r < 0) { orb.cy = orb.r; orb.vy = Math.abs(orb.vy) }
        if (orb.cy + orb.r > currentStageHeight) { orb.cy = currentStageHeight - orb.r; orb.vy = -Math.abs(orb.vy) }
      }
          ...
          {orbPositions.map((orb, i) => (
            <div
              ...
              style={{
                ...
                transition: orb.isDragging ? 'box-shadow 0.2s ease, background 0.2s ease' : 'box-shadow 0.4s ease, background 0.4s ease',
              }}
            />
          ))}
        </div>

        <h1 id="hero-title" className="sr-only">{HEADLINE_TEXT}</h1>
