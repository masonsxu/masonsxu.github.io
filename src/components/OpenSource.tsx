        ...
        <div className="mb-8 group relative bg-surface border border-border/20 rounded-lg overflow-hidden hover:border-primary/50 transition-colors">
          <div className="p-8">
            ...
            <div className="space-y-2 mb-6">
              <OSHighlight text={<>编写 <strong className="text-white">330+ 行 AGENTS.md</strong>，将架构规范、分层约束、错误处理模式等领域知识编码为 AI 可理解的结构化上下文</>} />
              <OSHighlight text={<>设计 <strong className="text-white">8 个 Custom Skills</strong> 封装端到端开发流程（如 /add-rpc-method 一键完成 IDL→DAL→Converter→Logic→Handler 全层实现 + 测试编写）</>} />
              <OSHighlight text={<>部署 <strong className="text-white">3 个 AI 驱动 GitHub Actions</strong>（Issue 自动实现 / PR 自动 Review / Issue 自动分类），实现 DevOps 自动化</>} />
            </div>
            <div className="flex flex-wrap gap-2">
              {['Kitex RPC', 'Hertz HTTP', 'Vibe Coding'].map(t => (
                <span key={t} className="text-xs px-2 py-1 bg-primary/10 border border-primary/20 rounded text-primary">{t}</span>
              ))}
              {['Etcd', 'Wire DI', 'Casbin', 'OpenTelemetry', 'AGENTS.md', 'GitHub Actions'].map(t => (
...
function OSHighlight({ text }: { text: React.ReactNode }) {
  return (
